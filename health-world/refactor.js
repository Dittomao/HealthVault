...
const content = fs.readFileSync('src/app/dashboard/DashboardClient.tsx', 'utf8');
              ...
              {!isUploading && !['jargon', 'timeline', 'report'].includes(activeTab) && (
                <motion.div key="generic" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }} className="bg-[#f4f4f4] rounded-[32px] p-8 md:p-10 text-center min-h-[400px] flex flex-col items-center justify-center">
                  ...
                  <div className="mt-8">
                     ...
                     <input id="file-input-generic" type="file" accept="image/*,application/pdf" className="hidden" onChange={(e) => handleFileUpload(e, activeTab)} />
                  </div>
                </motion.div>
              )}
...
const finalCode = beforeReturn + newReturnJSX;

fs.writeFileSync('src/app/dashboard/DashboardClient.tsx', finalCode);
console.log('Successfully refactored DashboardClient.tsx!');
